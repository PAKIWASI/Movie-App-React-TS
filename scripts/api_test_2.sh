...
# ─────────────────────────────────────────────
#  Movie App API — full test suite (HTTPie) v2
# ─────────────────────────────────────────────
...
# MOVIE_NAME_WORD=$(echo "$MOVIE_TITLE" | awk '{print $1}')
# req none GET "/api/movie?name=$MOVIE_NAME_WORD"
# assert "GET /api/movie?name= text search works"      '.success == true'
# assert "GET /api/movie?name= returns at least one"   '(.data | length) >= 1'

# ?id= query param — different code path from /:id (early-return branch)
req none GET "/api/movie?id=$TMDB_ID"
assert "GET /api/movie?id= returns the correct movie" ".data[0].id == $TMDB_ID" 
...
# ══════════════════════════════════════════════════════════════════════════════
section "Auth — refresh and logout"
# ══════════════════════════════════════════════════════════════════════════════

req "$SESSION_USER" POST "/api/auth/refresh"
assert "refresh: success"              '.success == true'

req "$SESSION_USER" GET "/api/user/me"
assert "refresh: new token works"      '.success == true'

req "$SESSION_USER" POST "/api/auth/logout"
assert "logout: success"               '.success == true'

req "$SESSION_USER" GET "/api/user/me"
assert "logout: access blocked → 401"  '.message | test("denied|token"; "i")'

req "$SESSION_USER" POST "/api/auth/refresh"
assert "logout: refresh dead → 401"    '.message | test("token|denied|invalid"; "i")'

# re-login so session is valid for the rest of the tests
req "$SESSION_USER" POST "/api/auth/login" \
    email="$USER_EMAIL" \
    password="Password123!"
...
# flags set at creation time should persist
req "$SESSION_USER" POST "/api/user/me/movie" \
    tmdbId:=999999901
# (movie doesn't exist → 404, proving the existence check fires)
assert "POST /user/me/movie: unknown tmdbId → 404" '.success == false'
...
# makeAdmin with nonexistent user
req "$SESSION_ADMIN" POST "/api/auth/admin/000000000000000000000001"
assert "makeAdmin: user not found → 404"         '.success == false'

# admin cannot delete another admin
req "$SESSION_ADMIN" DELETE "/api/user/$USER_ID"
assert "DELETE /user/:userid: cannot delete admin → 403" '.success == false'
...
# Both test users are admins — they must self-delete (admin can't delete admins)
